...
# --- Configuración de Path para Módulos Reutilizables ---
# Asegurar que el directorio del script actual esté en sys.path
# Esto es crucial para que Python encuentre módulos hermanos.
script_dir = os.path.dirname(os.path.abspath(__file__))
if script_dir not in sys.path:
    sys.path.insert(0, script_dir)

try:
    import liquidacion_supabase_handler as db_handler
except ImportError as e:
    st.error(f"Error crítico: No se pudo encontrar el `liquidacion_supabase_handler.py`. Asegúrate de que esté en la misma carpeta y que la ruta de importación sea correcta. Detalle: {e}")
    st.stop()
...
st.set_page_config(
    layout="wide",
    page_title="Liquidación de Lotes - INANDES",
    page_icon="💰"
)
...
if 'save_status_messages' not in st.session_state: st.session_state.save_status_messages = []
if 'operation_completed' not in st.session_state: st.session_state.operation_completed = False
...
# --- Funciones de Despliegue de Información (Replicadas de liquidacion_app.py) ---
...
def mostrar_busqueda():
    st.header("Paso 1: Buscar Lote para Liquidar")
    ...
    if submit_button:
        ...
        if not lote_id_sanitized:
            ...
            with st.spinner("Buscando facturas..."):
                resultados = db_handler.get_proposals_by_lote(lote_id_sanitized)
                st.session_state.lote_encontrado = resultados
                if resultados:
                    st.success(f"Se encontraron {len(resultados)} facturas activas.")
                    st.session_state.anexo_number = resultados[0].get('anexo_number', '')
                    st.session_state.contract_number = resultados[0].get('contract_number', '')
    ...
    if st.session_state.lote_encontrado:
        ...
        with st.form(key="select_invoices_form"):
            for f in st.session_state.lote_encontrado:
                st.checkbox(f"**Factura:** {parse_invoice_number(f['proposal_id'])} | **Emisor:** {f.get('emisor_nombre', 'N/A')}", 
            ...
            if st.form_submit_button("Cargar Historias para Liquidar"):
                ...
                if not ids:
                    ...
                    with st.spinner(f"Cargando detalles para {len(ids)} facturas..."):
                        detalles = [db_handler.get_proposal_details_by_id(pid) for pid in ids]
...
def mostrar_liquidacion():
    ...
    if st.button("<- Volver a la búsqueda"):
        ...
        st.session_state.save_status_messages = []
        st.rerun()
    ...
    with st.form(key="liquidation_form"):
        ...
        for i, f in enumerate(st.session_state.facturas_a_liquidar):
            with st.container(border=True):
                ...
                eventos_liquidacion = db_handler.get_liquidacion_eventos(f['proposal_id'])
                ...
                l_vars['monto_recibido'] = l_cols[1].number_input("Monto Recibido", l_vars['monto_recibido'], format="%.2f", key=f"mr_{i}")
        ...
        if st.form_submit_button("Liquidar Todas las Facturas", type="primary"):
            with st.spinner("Procesando liquidación en lote..."):
                lote_payload = []
                for factura in st.session_state.facturas_a_liquidar:
                    eventos_previos = db_handler.get_liquidacion_eventos(factura['proposal_id'])
                ...
                try:
                    response = requests.post(f"{API_BASE_URL}/liquidar_lote", json=lote_payload)
                    response.raise_for_status()
                    st.session_state.resultados_liquidacion_lote = response.json()
                    st.success("¡Lote liquidado con éxito!")
                    st.session_state.operation_completed = False # Resetear por si se liquida de nuevo
                    st.session_state.save_status_messages = []
                except requests.exceptions.RequestException as e:
    ...
    if st.session_state.resultados_liquidacion_lote:
        ...
        for i, f in enumerate(st.session_state.facturas_a_liquidar):
            ...
            res = st.session_state.resultados_liquidacion_lote['resultados_por_factura'][i]
        ...
        if st.session_state.save_status_messages:
            for msg in st.session_state.save_status_messages:
                if "✅" in msg:
                    st.success(msg)
                else:
                    st.error(msg)
        
        # --- Botones de Acción Post-Liquidación ---
        action_cols = st.columns(3)
        
        # Botón para subir a Supabase
        if not st.session_state.operation_completed:
            if action_cols[0].button("Subir Eventos a Supabase", type="primary"):
                st.session_state.save_status_messages = []
                all_saves_successful = True

                with st.spinner("Guardando eventos en Supabase..."):
                    for i, f in enumerate(st.session_state.facturas_a_liquidar):
                        res = st.session_state.resultados_liquidacion_lote['resultados_por_factura'][i]
                        loc_vars = f['local_liquidation_vars']
                        ...
                        try:
                            liquidacion_resumen_id = db_handler.get_or_create_liquidacion_resumen(proposal_id, f) 
                            if not liquidacion_resumen_id:
                                raise Exception(f"No se pudo obtener/crear resumen para {parse_invoice_number(proposal_id)}")
                            
                            db_handler.add_liquidacion_evento(
                                liquidacion_resumen_id=liquidacion_resumen_id,
                                tipo_evento=res.get('tipo_pago', 'Desconocido'),
                                fecha_evento=loc_vars['fecha_pago'], 
                                monto_recibido=loc_vars['monto_recibido'],
                                dias_diferencia=res.get('dias_diferencia', 0),
                                resultado_json=res 
                            )
                            
                            nuevo_saldo = res.get('liquidacion_final', {}).get('saldo_final_a_liquidar', 0)
                            db_handler.update_liquidacion_resumen_saldo(liquidacion_resumen_id, nuevo_saldo)
                            
                            st.session_state.save_status_messages.append(f"✅ Evento para factura {parse_invoice_number(proposal_id)} guardado con éxito.")
                        except Exception as e:
                            all_saves_successful = False
                            st.session_state.save_status_messages.append(f"❌ Error al guardar evento para factura {parse_invoice_number(proposal_id)}: {e}")
                    
                    st.session_state.operation_completed = all_saves_successful
                st.rerun()

        # Botón para generar PDF (siempre visible después de liquidar)
        if action_cols[1].button("Generar Reporte PDF"):
            with st.spinner("Generando reporte..."):
                # (El resto del código de generación de PDF permanece igual)
                report_data = {
                ...
                for i, f in enumerate(st.session_state.facturas_a_liquidar):
                    ...
                    factura_data = {
                        ...
                        'liquidation_params': f['local_liquidation_vars'],
                        'result': res
                    }
                ...
                temp_json_path = os.path.join(temp_dir, f"lote_report_data_{timestamp}.json")
                with open(temp_json_path, 'w', encoding='utf-8') as f:
                    json.dump(report_data, f, ensure_ascii=False, indent=4)

                output_filename = f"reporte_liquidacion_lote_{timestamp}.pdf"
                output_filepath = os.path.join(os.path.expanduser("~"), "Downloads", output_filename)
                generator_script_path = os.path.join(temp_dir, "lote_report_generator.py")

                command = ["python", generator_script_path, f"--data_file={temp_json_path}", f"--output_file={output_filepath}"]
                try:
                    result = subprocess.run(command, check=True, capture_output=True, text=True, encoding='utf-8')
                    if result.stderr:
                        st.warning(f"Advertencias del generador de PDF: {result.stderr}")
                    if os.path.exists(output_filepath):
                        st.success(f"Reporte generado: {output_filename}")
                        with open(output_filepath, "rb") as pdf_file:
                            st.download_button(label="Descargar Reporte PDF", data=pdf_file.read(), file_name=output_filename, mime="application/pdf")
                    else:
                        st.error(f"Error: El script generador no produjo un PDF. Salida: {result.stdout}")
                except subprocess.CalledProcessError as e:
                    st.error(f"Error al generar el PDF: {e.stderr}")
                finally:
                    if os.path.exists(temp_json_path):
                        os.remove(temp_json_path)

        # Botón para finalizar y volver (siempre visible después de liquidar)
        if action_cols[2].button("Finalizar y Volver"):
            st.session_state.vista_actual = 'busqueda'
            st.session_state.lote_encontrado = []
            st.session_state.facturas_a_liquidar = []
            st.session_state.resultados_liquidacion_lote = None
...
st.title("Módulo de Liquidación de Facturas por Lote")
